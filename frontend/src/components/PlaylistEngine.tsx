...
export const PlaylistEngine = ({ tracks, profile }: Props) => {
  ...
  const generate = () => {
    // Filter tracks by mood parameters using estimated values
    const scored = tracks.map((t: any, i: number) => {
      const p = Math.max(0.3, (t.popularity ?? 60) / 100);
      const energy = 0.35 + p * 0.45 + (tracks.length - i) / tracks.length * 0.15;
      const valence = 0.25 + p * 0.5;
      const tempo = 75 + p * 65;

      const energyScore = energy >= mood.energy[0] && energy <= mood.energy[1] ? 1 : 0;
      const valenceScore = valence >= mood.valence[0] && valence <= mood.valence[1] ? 1 : 0;
      const tempoScore = tempo >= mood.bpm[0] && tempo <= mood.bpm[1] ? 1 : 0;
      return { track: t, score: energyScore + valenceScore + tempoScore };
    });

    const sorted = scored.sort((a, b) => b.score - a.score || Math.random() - 0.5);
    setGenerated(sorted.slice(0, length).map(s => s.track));
  };
